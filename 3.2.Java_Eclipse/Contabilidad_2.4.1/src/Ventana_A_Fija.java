...
public class Ventana_A_Fija extends JDialog {
	...
	public static void main(String[] args) {
		try {
			Ventana_A_Fija dialog = new Ventana_A_Fija();
	 ...
	 * Create the dialog.
	 */
	public Ventana_A_Fija() {
		setBackground(new Color(255, 255, 255));
		setIconImage(Toolkit.getDefaultToolkit().getImage(Ventana_A_Fija.class.getResource("/Imagenes/Icon.png")));
		...
		contentPanel.setLayout(null);
		{
			...
			contentPanel.add(menuBar);
			{
				...
				menuBar.add(mnMenu);
				{
					...
					mntmValor.setIcon(new ImageIcon(Ventana_A_Fija.class.getResource("/Imagenes/liquidacion1.jpg")));
				...
				}
				{
					...
					mntmA_Variable.setIcon(new ImageIcon(Ventana_A_Fija.class.getResource("/Imagenes/indicador2.png")));
				...
				mntmA_Fija.setIcon(new ImageIcon(Ventana_A_Fija.class.getResource("/Imagenes/indicador1.png")));
			...
			}
			{
				...
				menuBar.add(mnAyuda);
				{
					...
					mntmCreditos.setIcon(new ImageIcon(Ventana_A_Fija.class.getResource("/Imagenes/Acerca de1.png")));
				...
				}
				{
					...
					mntmSalir.setIcon(new ImageIcon(Ventana_A_Fija.class.getResource("/Imagenes/Salir1.png")));
					mntmSalir.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent arg0) {
							int respuesta=JOptionPane.showConfirmDialog(null, "Esta seguro que desea cerrar la aplicacion?"," Valor Del Trabajador En Colombia",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
							if(respuesta==0)
								System.exit(0);
						}
					});
		...
		txtFinanciamiento.setBounds(178, 50, 65, 20);
		...
		txtAños.setBounds(178, 75, 89, 20);
		panel_1.add(txtAños);
		...
		lblPago.setBounds(178, 103, 89, 20);
		...
		lblNewLabel_5_1.setBounds(520, 28, 18, 14);
		...
		lblNewLabel_11.setBounds(82, 172, 46, 14);
		...
		lblNewLabel_12.setBounds(82, 201, 46, 14);
		...
		txtPR.setBounds(178, 169, 65, 20);
		panel_1.add(txtPR);
		txtPR.setColumns(10);
		
		txtDTF = new JTextField();
		txtDTF.setBounds(178, 198, 65, 20);
		panel_1.add(txtDTF);
		...
		lblTIO.setBounds(452, 147, 65, 20);
		panel_1.add(lblTIO);
		...
		lblNewLabel_5_1_1_2.setBounds(249, 172, 18, 14);
		...
		lblNewLabel_5_1_1_3.setBounds(249, 201, 18, 14);
		...
		comboBox.setBounds(178, 134, 89, 22);
		...
		lblNewLabel_14.setBounds(82, 138, 86, 14);
		...
		rdbtnVPN.setBounds(54, 80, 109, 23);
		panel.add(rdbtnVPN);
		...
		lblResultadoa.setBounds(237, 84, 63, 14);
		...
		lblResultadob.setBounds(237, 142, 63, 14);
		lblResultadob.setOpaque(true);
		panel.add(lblResultadob);
		
		JButton btnCalcular = new JButton("Calcular");
		btnCalcular.setBackground(new Color(240, 255, 240));
		btnCalcular.setIcon(new ImageIcon(Ventana_A_Fija.class.getResource("/Imagenes/Calcular1.png")));
		...
		btnCalcular.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(financia!=0) {
					...
					pago = (financia)*(((Math.pow(1+tasa_interes, periodo*año)*tasa_interes))/((Math.pow(1+tasa_interes, periodo*año)-1)));
				}else {
					pago = (inversion)*(((Math.pow(1+tasa_interes, periodo*año)*tasa_interes))/((Math.pow(1+tasa_interes, periodo*año)-1)));
				}
				
				Indicadores trabajador=new Indicadores(inversion,financia,año,interes,0,ingresos,gastos,valorrescue,pr,dtf,periodo);
				
				lblPago.setText(String.format("%.2f", pago));
				...
				amortizacion[1][1] = financia;
				amortizacion[1][2] = pago;
				amortizacion[1][3] = financia*tasa_interes;
				amortizacion[1][4] = pago-(financia*tasa_interes);
				...
				table.setValueAt(String.format("%.2f",amortizacion[1][1]), 1, 1);
				table.setValueAt(String.format("%.2f",amortizacion[1][2]), 1, 2);
				table.setValueAt(String.format("%.2f",amortizacion[1][3]), 1, 3);
				table.setValueAt(String.format("%.2f",amortizacion[1][4]), 1, 4);
				
				
				for( int i=2; i<=periodo*año; i++) {
					amortizacion[i][2] = pago;//columma A
					amortizacion[i][1] = amortizacion[i-1][1] - amortizacion[i-1][4];//columna p
					amortizacion[i][3] = amortizacion[i][1]*tasa_interes;//interes
					amortizacion[i][4] = amortizacion[i][2]-amortizacion[i][3];//abono p
				...
				for (int i=0; i<=periodo*año;i++) {
					DefaultTableModel modelo_2=(DefaultTableModel)t_flujo_caja2.getModel();
					String datos[]= {"","","",""};
					modelo_2.addRow(datos);
					}
				
				t_flujo_caja2.setValueAt(String.format("%.2f",tabla_ingresos[0]), 0, 1);
				t_flujo_caja2.setValueAt(String.format("%.2f",tabla_egresos[0]), 0, 2);
				t_flujo_caja2.setValueAt(String.format("%.2f",flujo_neto_efectivo[0]), 0, 3);
				
				
				
				for( int i=0; i<=periodo*año; i++) {                                                
					t_flujo_caja2.setValueAt(String.format("%.0f",amortizacion[i][0]), i, 0);
				}//periodos
				...
				for(int i=1;i<=periodo*año;i++) {
					...
					t_flujo_caja2.setValueAt(String.format("%.2f",tabla_ingresos[i]), i, 1);
					t_flujo_caja2.setValueAt(String.format("%.2f",tabla_egresos[i]), i, 2);
					t_flujo_caja2.setValueAt(String.format("%.2f",flujo_neto_efectivo[i]), i, 3);
				}//tabla flujo de caja
				
				//valor recuperado en la tablla
				...
				t_flujo_caja2.setValueAt(String.format("%.2f",flujo_neto_efectivo[periodo*año]), periodo*año, 3);
				t_flujo_caja2.setValueAt(String.format("%.2f",tabla_ingresos[periodo*año]), periodo*año, 1);
		...
		btnCalcular.setBounds(361, 80, 130, 47);
		panel.add(btnCalcular);
		
		JButton btnNuevo = new JButton("Nuevo");
		btnNuevo.setBackground(new Color(240, 255, 240));
		btnNuevo.setIcon(new ImageIcon(Ventana_A_Fija.class.getResource("/Imagenes/Nuevo1.png")));
		...
		btnNuevo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				DefaultTableModel model=(DefaultTableModel)t_flujo_caja2.getModel();
				model.setRowCount(0);
			}
		});
		btnNuevo.setBounds(361, 138, 130, 41);
		panel.add(btnNuevo);
		
		JPanel panel_2 = new JPanel();
		panel_2.setBackground(new Color(245, 255, 250));
		panel_2.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Amortizaci\u00F3n Fija", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		...
		t_flujo_caja2 = new JTable();
		t_flujo_caja2.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
		...
		scrollPane_1.setViewportView(t_flujo_caja2);
	}
}
